...
/**
 * SEMBOL YER TUTUCULARI örnek verinin sembolleriyle çözülüyor.
 *
 * Araç sembolü sabit yazıyordu (`X001`) ve bu yüzden yayındaki GERÇEK veride
 * hiç çalışmıyordu — o sembol orada yok, araç 120 saniye bekleyip düşüyordu.
 * Artık argümanla değiştirilebiliyor; burada örnek verinin sembolleri
 * konuyor, çünkü uçtan uca testler o veri setiyle koşuyor.
 */
...
for (const ekran of ekranlar) {
  test(`ölçüm aracının "${ekran.ad}" hazır ölçütü hâlâ geçerli`, async ({ page }) => {
    await page.goto(`/next.html?m=bist&${coz(ekran.url)}`, { waitUntil: 'networkidle' });
    await expect(
